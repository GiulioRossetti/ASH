...
class AttributeAnalysisCase(unittest.TestCase):
    ...
    def test_star_profile_homogeneity(self):
        ...
        for tid in a.temporal_snapshots_ids():
            for method in ['aggregate', 'collapse']:
                ...
                res = average_star_profile_homogeneity(a, tid=tid, method=method, by_label=False)
                self.assertListEqual(sorted(list(res.keys())), ["gender", "party"])
                res = average_star_profile_homogeneity(a, tid=tid, method=method, by_label=True)
                self.assertListEqual(sorted(list(res.keys())), ["gender", "party"])
    ...
    def test_average_group_degree(self):
        ...
        self.assertDictEqual(res, {'party': {'R': 2.0, 'L': 2.3333333333333335},
                                   'gender': {'M': 2.5, 'F': 2.0}})

        res = average_group_degree(a, tid=0, hyperedge_size=2)
        self.assertDictEqual(res, {'party': {'R': 1.0, 'L': 0.3333333333333333},
                                   'gender': {'M': 1.0, 'F': 0.0}})
        for tid in a.temporal_snapshots_ids():
