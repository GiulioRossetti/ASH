...
class AttributeAnalysisCase(unittest.TestCase):
    ...
    def test_profiles(self):
        a = ASH()
        a.add_node(1, start=1, end=5, attr_dict={'party':"L", 'age':37})
        ...
        self.assertEqual(
            sorted(hyperedge_aggregate_node_profile(a, "e1", 1).items()),
            [("age", 28.25),  ("party", "L")],
        )
        self.assertEqual(
    ...
    def test_hyperedge_profile_entropy(self):
        ...
        for tid in a.temporal_snapshots_ids():
            hes = a.hyperedges(start=tid, as_ids=True)
            for he in hes:
                res = hyperedge_profile_entropy(a, he, tid=tid)
                self.assertListEqual(sorted(list(res.keys())), ["gender", "party"])
